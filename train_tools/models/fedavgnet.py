...
def double_fedavg_cifar(num_classes=10):
    class DoubleFedAvgNetCIFAR(torch.nn.Module):
        def __init__(self, num_classes=10):
            super(DoubleFedAvgNetCIFAR, self).__init__()
            self.conv2d_1 = torch.nn.Conv2d(3, 32, kernel_size=5, padding=2)
            self.max_pooling = nn.MaxPool2d(2, stride=2)
            self.conv2d_2 = torch.nn.Conv2d(32, 64, kernel_size=5, padding=2)
            self.flatten = nn.Flatten()
        ...
        def forward(self, x, get_features=False):
            ...
            x = self.flatten(x)
            z = self.relu(self.linear_1(x))
            x1 = self.classifier(z)
            x2 = self.classifier_head(z)
            if get_features:
                return z, x1, x2

            else:
                return x1, x2
    return DoubleFedAvgNetCIFAR(num_classes=num_classes)