...
__all__ = ["resnet10", "resnet18", "resnet34", 'resnet18nonorm']
...
def resnet_pretrain18(num_classes=10):
    class PretrainResNet18(nn.Module):
        """
        ResNet
        """

        def __init__(self, shape_out=100):
            ...
            self.classifier = nn.Linear(512, shape_out)
    ...
    return PretrainResNet18(num_classes)

# cifar100
def double_resnet_pretrain18(num_classes=10):
    class PretrainResNet18(nn.Module):
        """
        ResNet
        """
        ...
        def __init__(self, shape_out=100):
            ...
            self.classifier = nn.Linear(512, shape_out)
            self.classifier_head = nn.Linear(512, shape_out)
        ...
        def forward(self, x, get_features=False):
            ...
            x = self.backbone.avgpool(x)
            feature = torch.flatten(x, 1)
            y1 = self.classifier(feature)
            y2 = self.classifier_head(feature)
            if get_features:
                return feature, y1, y2
            else:
                return y1, y2
...
def resnet8(num_classes=10):
    ...
    return ResNet_cifar(resnet_size=8, scaling=4,
                        save_activations=False, group_norm_num_groups=None,
                        freeze_bn=False, freeze_bn_affine=False, num_classes=num_classes)


def double_resnet8(num_classes=10):
    class ResNetBase(nn.Module):
...
def resnet18nonorm(num_classes=200):  # no norm and bias=True
    ...
    class ResNetMod(nn.Module):
        def __init__(self, num_classes):
            ...
            self.backbone = ResNet18NoNorm(num_classes=num_classes)

            self.prototype = nn.Linear(self.backbone.linear.in_features, num_classes, bias=True)
    ...
    return ResNetMod(num_classes)


def double_resnet18nonorm(num_classes=200):
    class BasicBlockNoNorm(nn.Module):
        expansion = 1
    ...
    class ResNetMod(nn.Module):
        def __init__(self, num_classes):
            ...
            self.classifer = nn.Linear(self.backbone.linear.in_features, num_classes, bias=True)
            self.classifer_head = nn.Linear(self.backbone.linear.in_features, num_classes, bias=True)
            self.backbone.linear = None

        def forward(self, x, get_features=False):
            # Convolution layers
            feature_embedding = self.backbone(x)
            logit_teacher = self.classifer(feature_embedding)
            logits_student = self.classifer_head(feature_embedding)
            if get_features:
                return feature_embedding, logit_teacher

            return logit_teacher, logits_student
