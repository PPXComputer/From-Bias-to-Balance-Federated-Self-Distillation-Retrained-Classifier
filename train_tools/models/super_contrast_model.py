...
__all__ = ['super_constrast_resnet8']


def super_constrast_resnet8(num_classes=10):
    ...
    class ResNet_cifar(ResNetBase):
        def __init__(
            ...
            dim_in = 64 * scaling * block_fn.expansion
            self.classifier = nn.Linear(
                in_features=int(dim_in),
                out_features=self.num_classes,
            )
            feat_dim = 1024
            self.classifier_head = nn.Sequential(nn.Linear(dim_in, dim_in), nn.BatchNorm1d(dim_in),
        ...
        def forward(self, x, get_features=False):
            ...
            y2 = nn.functional.normalize(self.classifier_head(x), dim=1)
