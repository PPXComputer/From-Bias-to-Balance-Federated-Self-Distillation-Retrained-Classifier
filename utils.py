...
def directory_setter(path="./results", make_dir=False):
    ...
    if not os.path.exists(path) and make_dir:
        ...
        logging.info("directory %s is created" % path)
...
def config_overwriter(opt, args):
    ...
    if args.lam1 is not None:
        opt.train_setups.algo.params.lam1 = args.lam1
    if args.lam2 is not None:
        opt.train_setups.algo.params.lam2 = args.lam2

    if args.tau is not None:
        opt.train_setups.algo.params.tau = args.tau

    if args.epoch_classifier is not None:
        opt.train_setups.algo.params.epoch_classifier = args.epoch_classifier

    if args.dyn_alpha is not None:
        opt.train_setups.algo.params.dyn_alpha = float(args.dyn_alpha)

    # 添加一个 alpha 值来控制当前的狄利克雷函数系数
    opt.data_setups.partition.data_alpha = args.data_alpha
